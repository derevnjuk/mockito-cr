...
- [How to work on Mockito](#how-to-work-on-mockito)
...
> ⚡ We use [husky](https://github.com/typicode/husky) and [commitlint](https://github.com/conventional-changelog/commitlint#readme), they will help you to follow these rules.
...
```bash
$ shards install --frozen --no-color
```
...
```bash
$ crystal build ./src/mockito.cr
```

The build artifacts will be stored in the root folder.

### Tests

Run this command in terminal to execute the tests via [Spec](https://crystal-lang.org/api/1.6.0/Spec.html):
...
This project uses [Ameba code quality analysis](https://github.com/crystal-ameba/ameba) for code linting.

> ⚡ The code analysis can be configured via `.ameba.yml` file in the project root folder.

To lint a project, run this command in terminal:
...
This project uses [crystal tool format](https://crystal-lang.org/reference/1.6/man/crystal/index.html#crystal-tool-format) for code formatting.

> ⚡ The formatter is not configurable by design. The idea is to avoid discussing or spending time choosing some formatting option when the language already chooses one for you.

To format a library, run this command in terminal:

```bash
$ crystal tool format --no-color
```
